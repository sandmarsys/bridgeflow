...
async function pushToScript(contacts, followups) {
  const res = await fetch(APPS_SCRIPT_URL, {
    method:"POST",
    body:JSON.stringify({ action:"sync", contacts:contactsToRows(contacts), followups:followupsToRows(followups) }),
  });
...
// ── PURE DISPLAY COMPONENTS (defined outside App so they never remount) ───────
...
function DetailView({ selected, contacts, followups, setFollowups, setContacts, setView, setForm, setEditMode, deleteContact, addLog, newLog, setNewLog, addFollowup, newFU, setNewFU, showFU, setShowFU, logRef }) {
...
export default function App() {
  ...
  const exportBackup=()=>{
    ...
    a.href=URL.createObjectURL(blob); a.download=`bridgeflow-${new Date().toISOString().split("T")[0]}.json`; a.click();
  ...
  const deleteContact=(id)=>{
    ...
    setFollowups(f=>f.filter(x=>x.contactId!==id));
    setView("contacts");
  };
  const addLog=(contactId)=>{
    ...
    setContacts(prev=>{
      const u=prev.map(c=>c.id===contactId?{...c,conversations:[entry,...(c.conversations||[])]}:c);
  ...
  return (
    <div style={{minHeight:"100vh",background:D.bg,fontFamily:"'DM Sans',sans-serif",color:D.text}}>
      ...
      <div style={{background:D.surface,borderBottom:`1px solid ${D.border}`,padding:"0 20px",display:"flex",alignItems:"center",height:52,gap:12}}>
        ...
        <div style={{flex:1}}/>
        <SyncDot/>
        <button onClick={()=>setShowSettings(true)} style={{background:D.card,border:`1px solid ${D.border}`,borderRadius:7,padding:"5px 12px",fontSize:13,color:D.textSub,cursor:"pointer",fontFamily:"inherit"}}>
          ⚙ Settings
        </button>
      </div>
      ...
      <div style={{maxWidth:740,margin:"0 auto",padding:"30px 20px"}}>
        ...
        {view==="detail"&&(
          <DetailView
            selected={selected} contacts={contacts} followups={followups}
            setFollowups={setFollowups} setContacts={setContacts} setView={setView}
            setForm={setForm} setEditMode={setEditMode} deleteContact={deleteContact}
            addLog={addLog} newLog={newLog} setNewLog={setNewLog}
            addFollowup={addFollowup} newFU={newFU} setNewFU={setNewFU}
            showFU={showFU} setShowFU={setShowFU} logRef={logRef}
          />
        )}
        {view==="add"&&(
          <AddEditView form={form} setForm={setForm} editMode={editMode} saveContact={saveContact} setView={setView}/>
        )}
      </div>

      {showSettings&&(
        <SettingsModal syncState={syncState} syncMsg={syncMsg} exportBackup={exportBackup} importBackup={importBackup} onClose={()=>setShowSettings(false)}/>
      )}
