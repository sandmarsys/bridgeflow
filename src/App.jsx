...
function rowsToContacts(rows) {
  ...
  return data.filter(r=>r[0]).map(r=>{
    ...
    o.cold=o.cold==="TRUE"; o.isPartner=o.isPartner==="TRUE"; o.isNetwork=o.isNetwork==="TRUE";
    return o;
  });
}
...
function ImportModal({contacts,warmContacts,mode,onImport,onClose}) {
  ...
  function confirmImport() {
    ...
    if(mode==="cold") {
      ...
      const newC=preview.map(p=>normalizeContact({...p,id:Date.now().toString()+Math.random().toString(36).slice(2),createdAt:new Date().toISOString(),conversations:[],stageEnteredAt:today,cadenceCompleted:[],cold:true,coldSince:today,coldFollowUpDate,stage:"Connection",isPartner:false,isNetwork:false}));
...
function PartnerBadge() {
  return (
    <span style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 10px",borderRadius:20,background:"#0D2010",color:"#4ADE80",fontSize:12,fontWeight:600,border:"1px solid #14532D"}}>
      🤝 Partner
    </span>
  );
}
function NetworkBadge() {
  return (
    <span style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 10px",borderRadius:20,background:"#1A1040",color:"#A78BFA",fontSize:12,fontWeight:600,border:"1px solid #3B2A7A"}}>
      🌐 Network
    </span>
  );
}
        ...
        <div>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {STAGES.map(st=>{const m=STAGE_META[st];const sel=form.stage===st;
              return (
                <button key={st} onClick={()=>setForm(f=>({...f,stage:st}))}
        ...
        <div style={{background:D.surface,borderRadius:10,padding:"14px 16px",border:`1px solid ${D.border}`}}>
          ...
          <div style={{display:"flex",gap:10,marginBottom:8}}>
            <button type="button" onClick={()=>setForm(f=>({...f,isPartner:!f.isPartner}))}
              ...
              🤝 Partner {form.isPartner&&<span style={{fontSize:11,background:"#4ADE8033",padding:"1px 7px",borderRadius:20}}>ON</span>}
            </button>
            <button type="button" onClick={()=>setForm(f=>({...f,isNetwork:!f.isNetwork}))}
              ...
              🌐 Network {form.isNetwork&&<span style={{fontSize:11,background:"#A78BFA33",padding:"1px 7px",borderRadius:20}}>ON</span>}
            </button>
          </div>
          <p style={{margin:0,fontSize:12,color:D.textMuted}}>Tags appear in the Partnership and Network tabs. Click to toggle.</p>
        </div>
        <div style={{display:"flex",gap:10}}>
...
function WarmView({warmContacts,setWarmContacts,contacts,setContacts,switchTab,showToast}) {
  ...
  function moveToPipeline() {
    if(!moveModal) return;
    const today=todayStr();
    const newContact=normalizeContact({...moveModal,id:Date.now().toString()+Math.random().toString(36).slice(2),createdAt:new Date().toISOString(),conversations:[],stageEnteredAt:today,cadenceCompleted:[],cold:false,coldSince:"",coldFollowUpDate:"",stage:moveStage,isPartner:false,isNetwork:false});
  ...
  return (
    <div>
          ...
          {warmContacts.map(w=>(
            <div key={w.id} style={{background:D.card,border:`1.5px solid ${D.warmBorder}`,borderRadius:12,padding:"13px 16px",display:"flex",alignItems:"center",gap:13}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:13,color:D.textSub,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{[w.title,w.coaching,w.company].filter(Boolean).join(" · ")||w.notes||""}</div>
      ...
      {showImport&&(
        <ImportModal contacts={contacts} warmContacts={warmContacts} mode="warm"
          onImport={(newW,skipped)=>{setWarmContacts(prev=>[...newW,...prev]);setShowImport(false);showToast(`${newW.length} contacts added to Warm list${skipped>0?`, ${skipped} duplicate${skipped>1?"s":""} skipped`:""}!`);}}
  ...
  return (
    <div>
      ...
      {partners.length===0?(
        <div style={{textAlign:"center",padding:"60px 20px",color:D.textMuted}}>
          <div style={{fontSize:40,marginBottom:10}}>🤝</div>
          <p style={{fontSize:14,marginBottom:8}}>No partners tagged yet.</p>
          <p style={{fontSize:13,color:D.textMuted}}>Open any contact, click Edit, and toggle the Partner tag.</p>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {partners.map(c=>{
            ...
            return (
              <div key={c.id} onClick={()=>{setSelected(c);setView("detail");}}
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:13,color:D.textSub,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{[c.title,c.company].filter(Boolean).join(" · ")}</div>
  ...
  return (
    <div>
      ...
      {network.length===0?(
        <div style={{textAlign:"center",padding:"60px 20px",color:D.textMuted}}>
          <div style={{fontSize:40,marginBottom:10}}>🌐</div>
          <p style={{fontSize:14,marginBottom:8}}>No network contacts tagged yet.</p>
          <p style={{fontSize:13,color:D.textMuted}}>Open any contact, click Edit, and toggle the Network tag.</p>
        </div>
      ):(
        <>
              ...
              return (
                <div key={c.id} onClick={()=>{setSelected(c);setView("detail");}}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:13,color:D.textSub,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{[c.title,c.coaching,c.company].filter(Boolean).join(" · ")}</div>
                    {lastNote&&<div style={{fontSize:12,color:D.textMuted,marginTop:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>💬 {lastNote.text}</div>}
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
...
function CalendarView({contacts,switchTab,calLinks,setCalLinks}) {
  ...
  const EventPopup=()=>{
    ...
    const ev=selectedEv; const evKey=makeEvKey(ev);
    const linkedId=calLinks[evKey]; const linked=linkedId?contacts.find(c=>c.id===linkedId):null;
  ...
  return (
    <div style={{minHeight:"100vh",background:D.bg,fontFamily:"'DM Sans',sans-serif",color:D.text}}>
      ...
      <div style={{background:D.surface,borderBottom:`1px solid ${D.border}`,padding:"0 20px",display:"flex",alignItems:"center",height:52,gap:12}}>
        ...
        <div style={{display:"flex",gap:2,background:D.card,borderRadius:8,padding:3,marginLeft:8,flexWrap:"wrap"}}>
